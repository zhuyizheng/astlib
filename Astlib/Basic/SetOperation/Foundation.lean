...
class HasFoundation extends HasEmpty M where
  memMin : M → M
  memMin_prop (x : M) (_ : x ≠ ∅) : memMin x ∈ x ∧ ¬∃ y ∈ x, y ∈ memMin x

export HasFoundation (memMin memMin_prop)
...
noncomputable instance [Extensional M] [HasEmpty M] (hM : M ⊨ M.L.foundation) :
  HasFoundation M where
  memMin := fun x ↦ Classical.choose (exists_memMin hM x)
  memMin_prop := fun x ↦ Classical.choose_spec (exists_memMin hM x)

theorem memMin_mem [HasFoundation M] {x : M} (hx : x ≠ ∅) : memMin x ∈ x :=
  memMin_prop x hx |>.left

@[simp]
theorem notMem_memMin [HasFoundation M] {x y : M} (hy : y ∈ x) :
    y ∉ memMin x := by
  have : x ≠ ∅ := by grind
  grind [memMin_prop]

@[simp]
theorem notMem_of_mem_memMin [HasFoundation M] {x y : M} (hy : y ∈ memMin x) :
    y ∉ x := by
  grind [notMem_memMin]

@[simp]
theorem notMem_self [HasUnorderedPair M] [HasFoundation M] : x ∉ x := by
  have := memMin_mem (x := ({x} : M)) (by simp)
  grind [memMin_mem, notMem_memMin]

@[simp]
theorem insert_neq [Extensional M] [HasUnorderedPair M] [HasSUnion M] [HasFoundation M] :
    succ x ≠ x := by
  grind [notMem_self]
