...
@[grind .]
theorem notin_empty [HasEmpty M] (x : M) : x ∉ (∅ : M) :=
  HasEmpty.empty_prop x

@[grind .]
theorem empty_subset [HasEmpty M] (x : M) : ∅ ⊆ x := by
  intro _; grind

-- export HasEmpty (empty)

-- @[inherit_doc] notation "∅"  => HasEmpty.empty
